...
//    }
    fileNameUrl = [NSURL URLWithString:fileName];
    player = [[AVPlayer alloc] initWithURL:fileNameUrl];
...
RCT_EXPORT_METHOD(play
                  : (nonnull NSNumber *)limitSec key
                  : (nonnull NSNumber *)key withCallback
                  : (RCTResponseSenderBlock)callback) {
    self.limitSeconds = [limitSec doubleValue];
    ...
    // 关闭定时器
    if(self.limitTimer){
        dispatch_source_cancel(self.limitTimer);
    }
    if(self.limitSeconds > 0){
        // 创建GCD定时器
        dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        self.limitTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, queue);
        dispatch_source_set_timer(self.limitTimer, dispatch_walltime(NULL, 0), 1.0 * NSEC_PER_SEC, 0); //每秒执行
        // 事件回调
        dispatch_source_set_event_handler(self.limitTimer, ^{
                if (player) {
                    if(player.rate != 0){
                        if(self.limitSeconds <= CMTimeGetSeconds(player.currentTime)){
                            [player pause];
                            if(callback){
                                callback([NSArray
                                arrayWithObjects:@"limit", nil]);
                            }
                        }
                    }
                }
        ...
        // 开启定时器
        dispatch_resume(self.limitTimer);
    }
}
...
RCT_EXPORT_METHOD(setCurrentTime
    ...
    if (value >= 0) {
        ...
        if (value == 0) {
            ...
            [player.currentItem
             seekToTime:CMTimeMakeWithSeconds([value doubleValue], 60000)
             completionHandler:^(BOOL finished) {
             }];
        }
    }
...
RCT_EXPORT_METHOD(getCurrentTime
    ...
    if (player) {
        callback([NSArray
            arrayWithObjects:[NSNumber numberWithDouble:CMTimeGetSeconds(player.currentTime)],
                             [NSNumber numberWithBool:player.rate != 0], nil]);
    } else {
...
RCT_EXPORT_METHOD(getCurrentDuration
                  : (nonnull NSNumber *)key withCallback
                  : (RCTResponseSenderBlock)callback) {
    AVPlayer *player = [self playerForKey:key];
    if (player) {
        callback([NSArray
            arrayWithObjects:[NSNumber numberWithDouble:CMTimeGetSeconds(player.currentItem.asset.duration)],
                             [NSNumber numberWithBool:player.rate != 0], nil]);
    } else {
        callback([NSArray arrayWithObjects:[NSNumber numberWithInteger:-1],
                                           [NSNumber numberWithBool:NO], nil]);
...
- (void)setOnPlay:(BOOL)isPlaying forPlayerKey:(nonnull NSNumber *)playerKey {
    RCTResponseSenderBlock callback = [[self callbackPool] objectForKey:playerKey];
    if(callback){
        callback([NSArray
